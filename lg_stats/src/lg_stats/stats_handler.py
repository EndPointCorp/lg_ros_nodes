...
class StatsHandler():
    ...
    def _handle_director(self, msg):
        ...
        if scene.get('slug', '') == 'attract-loop-break':
            ...
            # it there.
            print('ignoring attract-loop-break')
            return
    ...
    def handle_activity(self, msg):
        print(f'handling activity {msg.data}')
        if self.active_state == msg.data:
            print('ignoring duplicate active_state')
    ...
    def write_data(self):
        ...
        if self.last_presentation.get('slug', '') != 'attract-loop-break':
            ...
            write_influx_point_to_telegraf(f"jacobthing4 presentation_name=\"{pres['scene_name']}\",scene_name=\"{pres['scene_name']}\",type=\"{pres['type']}\",duration={duration},time_started={self.last_presentation_start_time}")
            write_influx_point_to_telegraf(f"jacobthin2 balogna=\"{pres['scene_name']}\",reason=\"jacob\",type=\"jacob\",user=\"jacobz\",val=1")
            print("wrote stats")
        else:
