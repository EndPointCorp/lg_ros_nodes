...
class TouchRouter:
    def __init__(self, event_pub, spacenav_viewport, default_viewport=None, non_multitouch_activities=None, non_multitouch_windows=None):
        self.event_pub = event_pub
        if default_viewport is None:
            self.default_viewports = set()
        else:
            self.default_viewports = set([default_viewport])
        # Any activities in this list will bypass touch diversion when present.
        self.non_multitouch_activities = [] if non_multitouch_activities is None else non_multitouch_activities
        # Any windows with activities in this list will be excluded from touch diversion.
        self.non_multitouch_windows = [] if non_multitouch_windows is None else non_multitouch_windows
    ...
    def handle_scene(self, publish_cb, scene):
        ...
        with self.lock:
            ...
            if not any((w['activity'] in self.non_multitouch_activities for w in windows)):
                ...
                rects = [absolute_geometry(w) for w in windows if w['activity'] not in self.non_multitouch_windows]
