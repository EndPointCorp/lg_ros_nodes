...
class LLM_KML:
    """
    Handles two parallel model calls:
      - One for a quick user-facing reply
      - One for a JSON-structured response of kml_tools calls
    Responds to the user and parses the JSON and triggers a callback with it.
    """
    ...
    def __call__(self, transcript: str) -> str:
        ...
        if self.debug:
            print("Quick reply:", quick_reply)
            print("JSON raw output:", json_raw)
...
def main():
    ...
    AGG_SYSTEM = os.getenv("AGG_SYSTEM", "LLM_KML")
