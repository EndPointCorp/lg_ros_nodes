...
if [[ ${SKIP_APT} != "" ]]; then
  ...
  lg-sudo-bg -w 'sudo apt-get update -q && sudo apt-get install gstreamer1.0-tools gstreamer1.0-plugins-base gstreamer1.0-plugins-good python-gst-1.0 libudev-dev gstreamer1.0-x ros-indigo-web-video-server -q -y'
  echo hi
fi
